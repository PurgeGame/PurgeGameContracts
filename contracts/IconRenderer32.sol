...
contract IconRenderer32 {
    ...
    function setCustomColorsForMany(
        ...
        for (uint256 i; i < count; ) {
            ...
            if (clearOutline) delete c.outline; else c.outline = outlineVal;
            if (clearFlame)   delete c.flame;   else c.flame   = flameVal;
            if (clearDiamond) delete c.diamond; else c.diamond = diamondVal;
            if (clearSquare)  delete c.square;  else c.square  = squareVal;
    ...
    function _resolve(
        ...
        { string storage s = _F(_custom[tokenId], k); if (bytes(s).length != 0) return s; }
        { string storage s = _F(_addr[owner_],  k);  if (bytes(s).length != 0) return s; }

        address coinAddr = coin;
        if (coinAddr.code.length != 0) {
            address ref = IPurgedRead(coinAddr).getReferrer(owner_);
            if (ref != address(0)) {
                { string storage s = _F(_addr[ref], k); if (bytes(s).length != 0) return s; }
    ...
    function _ownerOf(uint256 tokenId) private view returns (address a) {
        ...
        (bool ok, bytes memory ret) =
            g.staticcall(abi.encodeWithSelector(0x6352211e, tokenId)); // ownerOf(uint256)
        if (!ok || ret.length < 32) return address(0);
        assembly { a := mload(add(ret, 32)) }
    }
    ...
    function tokenURI(
        ...
        // ----- Regular token path -----
        lvl = uint24((data >> 24) & 0xFFFFFF);
        uint16 lastEx = uint16((data >> 48) & 0xFFFF);    // 0..255 valid; 420 = sentinel “none”
        uint24 traits = uint24(data);
    ...
    /// @param quadPos   Visual quadrant position index: 0=BL, 1=BR, 2=TL, 3=TR.
    /// @param quadId    Data quadrant id (0..3) used for symbol selection and sizing.
    /// @param colorIndex Palette color index (0..7).
    /// @param symbolIndex Symbol index within the quadrant (0..7).
    /// @param liveRemaining Live remaining supply for this trait in the current level.
    /// @param lastExterminated Last exterminated trait id (0..255) or 420 sentinel.
    ...
    function _svgQuad(
        ...
        bool highlightInvert = (lastExterminated != 420 && lastExterminated <= 255 && traitId == uint8(lastExterminated));
        ...
        string memory ringsSvg = _rings(
            ...
            "#111",
            "#fff",
            rOuter, rMiddle, rInner, CX[quadPos], CY[quadPos]
        );
        ...
        uint16 vbMax = vbW > vbH ? vbW : vbH; if (vbMax == 0) vbMax = 1;
        ...
        string memory symbolSvg = (quadId == 0)
            ? string(abi.encodePacked(
                '<g transform="', _mat6(scale1e6, txMicro, tyMicro),
                '"><g style="vector-effect:non-scaling-stroke">', iconPath, "</g></g>"
              ))
            : string(abi.encodePacked(
                '<g transform="', _mat6(scale1e6, txMicro, tyMicro),
                '"><g fill="', COLOR_HEX[colorIndex], '" stroke="', COLOR_HEX[colorIndex],
                '" style="vector-effect:non-scaling-stroke">', iconPath, "</g></g>"
              ));
    ...
    function _symTitle(uint256 quadId, uint8 symbolIndex) private view returns (string memory) {
        ...
        unchecked { return (uint256(symbolIndex) + 1).toString(); }
    }
    ...
    function _descFromRem(
        ...
        return string(
            abi.encodePacked(
                _label(0, col[0], sym[0]), " : ", uint256(rem[0]).toString(), " remaining\\n",
                _label(1, col[1], sym[1]), " : ", uint256(rem[1]).toString(), " remaining\\n",
                _label(2, col[2], sym[2]), " : ", uint256(rem[2]).toString(), " remaining\\n",
                _label(3, col[3], sym[3]), " : ", uint256(rem[3]).toString(), " remaining"
            )
        );
    }
    // ---------------- Trophy / SVG helpers -----------------------------------------------------------
    ...
    * - No inversion/negative rendering here (per spec: only one *badge* may invert in _svgQuad()).
    * - `Icons32` assets are indexed strictly by `dataQ∈[0..3]` and `symIdx∈[0..7]` → index in [0..31].
    */
    function _trophySvg(uint256 tokenId, uint16 exterminatedTrait) private view returns (string memory) {
        // --- Unwon/placeholder trophy (sentinel 0xFFFF) ---------------------------------------------
        if (exterminatedTrait == 0xFFFF) {
            uint8 ringIdx = 3; // Red
            // Border resolves from token → owner → referrer chain; default falls back to palette.
            ...
            // Static radii for trophy; downstream geometry reuses standard ratios.
            uint32 rOut = 44;
            ...
            string memory flame = string(
                abi.encodePacked(
                    '<g clip-path="url(#ct)">',
                        '<path fill="', _resolve(tokenId, /*flame*/1, "#111"),
            ...
            // No inversion here; trophies are never “negative”.
            return string(abi.encodePacked(head, rings, clip, flame, _svgFooter()));
        }

        // --- Won trophy ------------------------------------------------------------------------------

        // Trait namespace: high 2 bits select quadrant group; low 6 bits select the specific trait.
        ...
        // Border resolves with trait context; falls back gracefully if not customized.
        string memory border = _resolve(
            tokenId,
            /*k=*/0,
            _borderColor(tokenId, /*traitsPacked=*/uint24(six), _repeat4(colIdx))
        );

        // Slightly smaller ring for won trophy (visual distinction).
        uint32 rOut2 = 38;
        ...
        string memory body = string(
            abi.encodePacked(
                '<g fill="', COLOR_HEX[colIdx], '" stroke="', COLOR_HEX[colIdx],
                '" style="vector-effect:non-scaling-stroke">', g, "</g>"
            )
        );
        ...
        return string(
            abi.encodePacked(
                ...
                _rings(
                    ...
                    rOut2, rMid2, rIn2, /*cx=*/0, /*cy=*/0
                ),
                '<defs><clipPath id="ct2"><circle cx="0" cy="0" r="', uint256(rIn2).toString(), '"/></clipPath></defs>',
                '<g clip-path="url(#ct2)">',
                    '<g transform="', _mat6(sSym1e6, txm, tyn), '">', body, "</g>",
                "</g>",
                _svgFooter()
            )
        );
        ...
        return string(
            abi.encodePacked(
                ...
                '<defs><filter id="inv" color-interpolation-filters="sRGB">',
                    ...
                    '"/>',
                '</filter></defs>',
                '<rect x="-50" y="-50" width="100" height="100" rx="12" fill="', squareFill,
    ...
    */
    function _guides(
        ...
        return string(
            abi.encodePacked(
                '<g stroke="', borderColor, '" stroke-width="1" fill="none" opacity="1">',
                    '<line x1="0" y1="-50" x2="0" y2="50"/>',
                    '<line x1="-50" y1="0" x2="50" y2="0"/>',
                '</g>',
                '<path d="M0,15.5 L15.5,0 0,-15.5 -15.5,0 Z" fill="', diamondFill,
        ...
        return string(
            abi.encodePacked(
                ...
                '<g clip-path="url(#cd)"><g transform="translate(-1.596,-1.664) scale(2)">',
                    '<path fill="', flameFill, '" stroke="none" transform="matrix(0.027 0 0 0.027 -10.8 -8.10945)" d="',
                        DIAMOND_LOGO_PATH, '"/>',
                '</g></g>'
            )
        );
    ...
    *
    * @param tokenId  Token id (only used for non‑trophy naming).
    * @param isTrophy True = name as a level trophy; false = regular token name with #tokenId suffix.
    * @param svg      Raw SVG markup (not encoded). Will be base64‑encoded here.
    * @param level    Current level to display in "name".
    * @param desc     Human‑readable description. (Assumed controlled/no quotes.)
    *
    * Security:
    * - `desc` is composed internally (no user input). If you later introduce user text,
    *   escape JSON special characters before concatenation.
    */
    function _pack(
        uint256 tokenId,
        bool isTrophy,
    ...
    */
    ...
    /**
    * @dev Compose a global 0..255 trait id from quadrant (dataQ∈[0..3]), color index (colIdx∈[0..7]),
    *      and symbol index (symIdx∈[0..7]).
    *
    * Layout:
    *   dataQ << 6  | (colIdx << 3) | symIdx
    *   [7:6]          [5:3]           [2:0]
    */
    ...
    /**
    * @dev Read the starting “remaining” supply for the (quadrant,color,symbol) trait bucket.
    *      Value is seeded once per level via setStartingTraitRemaining(..).
    */
    ...
    *      - start<=1 → 1_000_000 (no scaling edge case)
    *      - curr clamped to [1..start]
    *      - linear ramp from 500_000 (start) to 1_000_000 (curr==1)
    *
    * Returns:
    *   factor in [500_000 .. 1_000_000]
    */
    ...
    /**
    * @dev Unified ring painter: draws three concentric circles (outer/mid/inner) centered at (cx,cy).
    *      Colors must be sanitized beforehand (renderer enforces strict “#rrggbb” where applicable).
    */
    function _rings(
        string memory outer,
        string memory mid,
        ...
        return string(
            abi.encodePacked(
                '<circle cx="', cxs, '" cy="', cys, '" r="', uint256(rOut).toString(), '" fill="', outer, '"/>',
                '<circle cx="', cxs, '" cy="', cys, '" r="', uint256(rMid).toString(), '" fill="', mid,   '"/>',
                '<circle cx="', cxs, '" cy="', cys, '" r="', uint256(rIn ).toString(), '" fill="', inner, '"/>'
            )
        );
    }

    /**
    * @dev Signed small‑int to string (for SVG positions). Minimal and safe for int16 domain.
    */
    ...
    *
    * Assumptions:
    * - `pos` ∈ {0,1,2,3}. Caller (quad painter) guarantees this.
    */
    ...
    *      Defaults to 95% of base; specific glyphs get small +/- adjustments.
    *
    * @param dataQ  Quadrant group (0..3)
    * @param symIdx Symbol index within the group (0..7)
    * @return f     1e6‑scaled multiplier (≈ 0.80–1.03 of base)
    */
    function _symbolFit1e6(uint256 dataQ, uint8 symIdx) private pure returns (uint32) {
        ...
        if (dataQ == 1 && symIdx == 6) {
            ...
            // BL / Mushroom
            f = uint32((uint256(f) * 115_000) / 100_000);
    ...
    *
    * @param s1e6   Scale in 1e6 fixed point
    * @param tx1e6  X translation in 1e6 fixed point (can be negative)
    * @param ty1e6  Y translation in 1e6 fixed point (can be negative)
    */
    ...
    */
    ...
    // NOTE: uses the signed small‑int helper `_i(int16)` defined earlier in this contract.
    // Do not re‑declare it here to avoid duplication.
    ...
    *
    * @param tokenId Token id (for deterministic seed)
    * @param traits  Packed 24‑bit traits (for deterministic seed)
    * @param used    The 4 color indices already used in the quadrants
    */
    function _borderColor(uint256 tokenId, uint24 traits, uint8[4] memory used) private view returns (string memory) {
        ...
        for (uint8 i; i < 8; ++i) {
            ...
            bool isUsed;
            for (uint8 j; j < 4; ++j) {
                if (used[j] == idx) { isUsed = true; break; }
            }
    ...
    /**
    * @dev Decode 24‑bit packed traits (4×6‑bit) into color and symbol indices per quadrant.
    *      For each quadrant q: v = (traits >> (q*6)) & 0x3F; col = v>>3; sym = v&7.
    */
    function _decodeTraits(uint24 t) private pure returns (uint8[4] memory col, uint8[4] memory sym) {
        unchecked {
            for (uint256 q; q < 4; ++q) {
                uint8 v = uint8((t >> (q * 6)) & 0x3F);
                col[q] = v >> 3;
                sym[q] = v & 0x07;
            }
        }
