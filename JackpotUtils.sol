...
/**
 * @title JackpotUtils
 * @notice Pure/view helpers for drawing winners from per-trait ticket buckets and
...
library JackpotUtils {
    ...
    function _randTraitTicket(
        mapping(uint8 => address[]) storage traitPurgeTicket,
        uint256 randomWord,
        uint8   trait,
        uint8   numWinners,
        uint8   salt
        ...
        uint256 k = numWinners < n ? numWinners : n;
        winners = new address[](k);

        // Deterministic base seed per (randomWord, salt, trait).
        bytes32 base = keccak256(abi.encode(randomWord, salt, trait));

        // Independent draws (with replacement). If uniqueness is required,
        // switch to a distinct-draw sampler at the call site.
        for (uint256 i; i < k; ) {
